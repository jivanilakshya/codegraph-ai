...
export function RepositoryTree({ className, files, selectedFileId, isLoading, onSelectFile, onRefresh }: RepositoryTreeProps) {
  const tree = createTree(files);
  return (
    <aside className={`flex min-h-0 flex-col bg-[#0a1019] ${className ?? "h-[min(50vh,24rem)] border-b border-slate-800 lg:h-auto lg:border-b-0 lg:border-r"}`}>
