    ...
    <article className="group flex min-h-72 flex-col rounded-xl border border-slate-800 bg-slate-950/70 p-5 shadow-[0_16px_40px_-30px_rgba(0,0,0,0.95)] transition hover:-translate-y-0.5 hover:border-slate-700 hover:bg-slate-950">
      ...
      <dl className="mt-5 space-y-3 border-y border-slate-800/80 py-4 text-sm">
        <div className="flex items-center justify-between gap-4">
          <dt className="flex items-center gap-2 text-slate-500"><GitBranch className="size-4" /> Default branch</dt>
          <dd className="truncate font-mono text-slate-300">{project.default_branch ?? "—"}</dd>
        </div>
        <div className="flex items-center justify-between gap-4">
      ...
      <div className="mt-auto flex gap-2 pt-5">
        <Link
          ...
          className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 text-sm font-medium text-slate-200 transition-colors hover:bg-slate-800"
        >
        ...
        <button
          ...
          disabled={isBusy}
          className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-lg bg-cyan-400 px-3 text-sm font-semibold text-slate-950 transition-colors hover:bg-cyan-300 disabled:cursor-wait disabled:opacity-70"
        >
          {isScanning ? <LoaderCircle className="size-4 animate-spin" /> : <Play className="size-4" />}
        ...
        <button
          ...
          className="grid size-9 place-items-center rounded-lg border border-slate-800 text-slate-500 transition-colors hover:border-rose-500/40 hover:bg-rose-500/10 hover:text-rose-300 disabled:cursor-wait disabled:opacity-40"
        >
