...
export function GraphInspector({ className, edges, node, nodesById, onClose, onNodeSelect, projectName }: GraphInspectorProps) {
  ...
  return <aside aria-label="Node inspector" className={`overflow-y-auto border-slate-800 bg-slate-950/95 p-4 backdrop-blur-md ${className ?? "max-h-[min(72vh,48rem)] rounded-xl border"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-cyan-300">Node Inspector</p><h2 className="mt-1 truncate text-lg font-semibold text-slate-100" title={node.label}>{node.label}</h2></div><button type="button" onClick={onClose} className="rounded-md p-1 text-slate-500 hover:bg-slate-800 hover:text-slate-200" aria-label="Close node inspector"><X className="size-4" /></button></div><dl className="mt-4 space-y-2 rounded-lg border border-slate-800 bg-slate-900/40 p-3 text-sm"><div className="flex justify-between gap-3"><dt className="text-slate-500">Type</dt><dd className="font-medium capitalize text-slate-200">{node.type}</dd></div>{owningFile ? <div className="flex justify-between gap-3"><dt className="text-slate-500">File</dt><dd className="max-w-40 truncate font-medium text-slate-200" title={owningFile.label}>{owningFile.label}</dd></div> : null}{parentArchitectureNode ? <div className="flex justify-between gap-3"><dt className="text-slate-500">Parent</dt><dd className="max-w-40 truncate font-medium text-slate-200" title={parentArchitectureNode.label}>{parentArchitectureNode.label}</dd></div> : null}<div className="flex justify-between gap-3"><dt className="text-slate-500">Project</dt><dd className="max-w-40 truncate font-medium text-slate-200">{projectName ?? "Current project"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Connected</dt><dd className="font-medium text-slate-200">{incoming.length + outgoing.length}</dd></div></dl><div className="mt-4 space-y-4">{declaredIn && owningFile ? <section className="border-t border-slate-800 pt-4"><h3 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500"><FileCode2 className="size-3.5" />Declared In</h3><button type="button" onClick={() => onNodeSelect(owningFile.id)} className="mt-2 w-full rounded-md border border-slate-800 bg-slate-900/50 px-2.5 py-2 text-left transition-colors hover:border-cyan-500/50 hover:bg-cyan-400/10 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"><p className="truncate text-sm font-medium text-slate-200">{owningFile.label}</p></button></section> : null}<RelationshipList title="Contains" icon={ArrowDownLeft} edges={contains} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Calls" icon={ArrowUpRight} edges={calls} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Called By" icon={ArrowDownLeft} edges={calledBy} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Extends" icon={ArrowUpRight} edges={extendsEdges} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Extended By" icon={ArrowDownLeft} edges={extendedBy} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Methods" icon={ArrowUpRight} edges={hasMethods} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Method Of" icon={ArrowDownLeft} edges={methodOf} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Imports" icon={FileCode2} edges={imports} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Imported By" icon={ArrowDownLeft} edges={importedBy} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /><RelationshipList title="Declares" icon={FileCode2} edges={declares} node={node} nodesById={nodesById} onNodeSelect={onNodeSelect} /></div></aside>;
}
